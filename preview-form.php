...
if ($result->num_rows === 0) {
    ...
    if (empty($doc['file_content'])) {
        ...
        error_log("Fetched document: ID=$id, File Name={$doc['file_name']}, File Type={$doc['file_type']}, Content Length=" . strlen($doc['file_content']) . " bytes");
    }
}
$stmt->close();
$conn->close();
                        ...
                        <div class="bg-white rounded-lg shadow-sm p-6">
                            <div class="space-y-6">
                                ...
                                <div class="form-group">
                                    ...
                                    <div class="text-sm text-gray-700"><?php echo number_format(strlen($doc['file_content']) / 1024, 2) . ' KB'; ?></div>
                                </div>
                                ...
                                <div class="preview-container">
                                    ...
                                    if ($doc['file_type'] === 'application/pdf') {
                                        echo '<iframe src="data:application/pdf;base64,' . $base64Content . '" class="preview-pdf"></iframe>';
                                    } elseif (in_array($doc['file_type'], ['image/jpeg', 'image/png'])) {
                                        echo '<img src="data:' . htmlspecialchars($doc['file_type']) . ';base64,' . $base64Content . '" class="preview-image" alt="Document Preview">';
                                        echo '<div id="imageError" class="error" style="display: none;">Error loading image. The file might be corrupted or invalid.</div>';
                                    } else {
                                        echo '<div class="error">Unsupported file type for preview.</div>';
                                    }
                                    ?>
                                </div>
